...
func ExampleCMS_UploadAssetDirectly_normal() {
	...
	assetID, err := c.UploadAssetDirectly(ctx, cmsProject, "test.txt", body)
	if err != nil {
...
func ExampleCMS_CreateAssetUpload_normal() {
	ctx := context.Background()
	c, err := cms.New(cmsURL, cmsToken)
	if err != nil {
		panic(err)
	...
	upload, err := c.CreateAssetUpload(ctx, cmsProject, "test.txt")
	if err != nil {
		panic(err)
	}

	if err := c.UploadToAssetUpload(ctx, upload, body); err != nil {
		panic(err)
	}
